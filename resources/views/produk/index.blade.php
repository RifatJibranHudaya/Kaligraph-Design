...
<div class="grid grid-2">
  ...
  <div class="card">
    ...
    <form method="POST" action="{{ route('produk.store') }}" enctype="multipart/form-data">
      ...
      <div class="form-group">
        ...
        <input type="text" name="nama" class="form-control" placeholder="cth. Neon Box Akrilik LED Bulat" value="{{ old('nama') }}" required>
      </div>
  ...
  <div class="card">
    <div class="card-header">
      <h3 class="card-title">Katalog Produk ({{ $products->count() }})</h3>
    </div>
          ...
          @forelse($products as $p)
            <tr>
              ...
              <td>
                ...
                @endif
                <div style="font-size:11px; color:var(--text-muted); line-height:1.4; max-width:240px; margin-top:2px;">
              ...
              <td>
                <div style="display:flex; gap:6px;">
                  <button type="button" class="btn btn-sm btn-secondary" onclick="openEditProductModal({{ $p->id }}, '{{ addslashes($p->nama) }}', '{{ $p->category_id ?? '' }}', {{ $p->harga_min ?: $p->harga_default }}, {{ $p->harga_max ?: 'null' }}, '{{ addslashes($p->deskripsi ?? '') }}', '{{ $p->foto_url ?? '' }}')">
                    ✏️
                  </button>
...
function openEditProductModal(id, nama, categoryId, hargaMin, hargaMax, deskripsi, fotoUrl) {
